...
We use [camelCase](https://en.wikipedia.org/wiki/Camel_case) for fields, file names and query parameters. All abbreviations SHOULD be capitalized (unless a domain wide standard dictates otherwise). If the identifier starts with an abbreviation it SHOULD be lowercase when the context dictates the use of camelCase (e.g. such as in field names) and uppercase when the context dictates PascalCase (e.g. such as in type names). Examples: `customerID` ✅, `JSONSchema` ✅, `{ "id": 1 }` ✅, `customerId` 🚫, `JsonSchema` 🚫.
